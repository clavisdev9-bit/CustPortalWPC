...
import { listOrderLines, listOrderMessages, postOrderMessage } from '../api/orders';
import SafeHtml from './SafeHtml';

// Dropped inline under an order's own row, same placement rule as DocumentsPanel -- both the
// product breakdown and the communication history are scoped to one order. Mirrors
// QuotationDetailPanel (same product-table + chatter shape on sale.order); kept as a separate
// component rather than a shared one because the two pages already have separate api modules
// (quotations.js / orders.js) with the same one-function-per-endpoint convention.
export default function OrderDetailPanel({ orderId }) {
  ...
  async function handleSend(e) {
    ...
    setSendError(null);
    try {
      setMessages(await postOrderMessage(orderId, body));
      setDraft('');
    } catch (err) {
  ...
  return (
    <div className="quotation-detail-panel">
      <div>
        ...
        {lines && lines.length === 0 && <p className="muted">No line items on this order.</p>}
